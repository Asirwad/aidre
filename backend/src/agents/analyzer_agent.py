...
from ..data import get_cortex_llm
...
async def analyze_node(state: AgentState) -> Dict:
    ...
    """
    try:
        llm = get_cortex_llm(temperature=0.0)  # Deterministic for analysis
        ...
        for table_name in state.table_metadata.keys():
            try:
                ...
                stats = state.table_stats.get(table_name)
                
                # Prepare data for LLM
                metadata_dict = metadata.model_dump() if hasattr(metadata, 'model_dump') else metadata.dict()
                stats_dict = stats.model_dump() if stats and hasattr(stats, 'model_dump') else (stats.dict() if stats else {})
...
Statistics: {json.dumps(stats_dict, default=str)}
"""
